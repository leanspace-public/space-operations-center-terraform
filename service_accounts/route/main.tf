...
resource "leanspace_access_policies" "default_route_permission_docker_compose_suspend_side_car" {
  name        = "docker-compose permission for suspend sidecar"
  description = "Permission use in docker-compose to suspend all route for TM/TC"
  statements {
    name = "suspend_search route"
    actions = [
      "routes:searchRoutes",
...
resource "leanspace_service_accounts" "route_service_account_docker_compose_suspend_side_car" {
  name       = "docker-compose permission for suspend sidecar"
  policy_ids = [leanspace_access_policies.default_route_permission_docker_compose_suspend_side_car.id]
}
